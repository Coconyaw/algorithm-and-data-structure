// AOJ ALDS1_7_C: http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=ALDS1_7_C
...
func (t tree) preParse(id int) {
	fmt.Printf(" %d", id)
	if t[id].left != NULL {
		t.preParse(t[id].left)
	}
	if t[id].right != NULL {
		t.preParse(t[id].right)
	}
}

// Inorder tree walk.
// left subtree -> root -> right subtree
func (t tree) inParse(id int) {
	if t[id].left != NULL {
		t.inParse(t[id].left)
	}
	fmt.Printf(" %d", id)
	if t[id].right != NULL {
		t.inParse(t[id].right)
	}
}

// Postorder tree walk.
// left subtree  -> right subtree -> root
func (t tree) postParse(id int) {
	if t[id].left != NULL {
		t.postParse(t[id].left)
	}
	if t[id].right != NULL {
		t.postParse(t[id].right)
	}
	fmt.Printf(" %d", id)
}
